...
class GaitOpticalFlowDataset(Dataset):
    def __init__(self, dataframe, transform=None, label_to_index=None,
                 return_metadata=False, image_size=(224, 224), flow_augment=None):
        self.data = dataframe
        self.transform = transform or transforms.Compose([
            transforms.Resize(image_size),
            transforms.ToTensor(),
    ...
    def __getitem__(self, idx):
        ...
        accumulated_flow = torch.zeros(2, H, W)
        for i in range(len(frames) - 1):
            flow = self.compute_optical_flow(frames[i], frames[i + 1])
            accumulated_flow += flow

        if self.flow_augment:
            accumulated_flow = self.flow_augment(accumulated_flow)

        if self.return_metadata:
            meta = {k: item[k] for k in ['subject', 'angle', 'trial', 'source']}
            return accumulated_flow, label, meta

        return accumulated_flow, label
